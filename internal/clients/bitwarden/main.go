...
// BW wraps utility methods to interacts with bw CLI
type BW struct {
	foldersIDs []FolderID
...
// If more than one folder match with folderName the first one will be chose
func (bw *BW) SetFoldersIDs(foldersNames []string) error {
	for _, name := range foldersNames {
		...
		if err != nil {
			return fmt.Errorf(
				"env-secrets error: an error occurred while setting folderIDs: %v",
				err,
			)
		}
		if len(results) == 0 {
...
func (bw *BW) FetchItems() error {
	if len(bw.foldersIDs) == 0 {
		panic("unable to fetch from unknown folder, please set foldersIDs before")
	}
	for _, id := range bw.foldersIDs {
		var results []Item
		err := common.ExecCLI(
			bwBinary,
			[]string{"list", "items", "--folderid", string(id)},
			&results,
		)
		if err != nil {
			return fmt.Errorf(
				"env-secrets error: an error occurred fetching items in folder with id %s: %v",
				id,
				err,
			)
		}
